...
export class Web3Service {
  ...
  async purchaseProduct(productId: number): Promise<boolean> {
    if (!this.contract) {
      toast.error('Contract not connected');
      return false;
    }
    ...
    try {
      ...
      const totalAmount = ethers.parseEther((parseFloat(product.basePrice) + parseFloat(product.carbonTax)).toString());
      ...
      toast.success(`Purchase successful! TX: ${receipt.hash}`, { id: 'purchase' });
      return true;
    } catch (error) {
  ...
  async stakeTokens(amount: string): Promise<boolean> {
    ...
    try {
      ...
      toast.error('Staking failed', { id: 'stake' });
      return false;
    }
  }
